...
class Mo_Ldap_Local_Login
{
            ...
            <table class="form-table">
                ...
                foreach ($wp_options as $option => $value) {
                    if (strpos($option, "mo_ldap_local_custom_attribute_") === false) {
                        ...
                        ?>
                        <tr>
                        <td><b><font color="#FF0000"></font><?php echo esc_attr($value); ?></b></td>
                        <td><input type="text" name="<?php echo esc_attr($option); ?>"
                                   value="<?php echo esc_attr(get_user_meta($user->ID, $option, true)); ?>" style="width:61%;"/>
                        </td>
                        </tr><?php
                    }
                }
    ...
    function ldap_login($user, $username, $password)
    {
        ...
        if ($auth_response->statusMessage == 'LDAP_USER_BIND_SUCCESS') {
            ...
            if (username_exists($username) || email_exists($username)) {
                ...
                if($mo_ldap_local_ldap_username_attribute=="samaccountname")
                {
                    if(empty($profile_attributes['mail']))
                ...
                if (!get_option('mo_ldap_local_register_user')) {
                    ...
                    $fname = !empty($profile_attributes['fname'])?$profile_attributes['fname']:'';
                    $lname = !empty($profile_attributes['lname'])?$profile_attributes['lname']:'';
    ...
    function create_customer() {
        ...
        if (!empty($customerKey)) {
            ...
            } elseif (strcasecmp($customerKey['status'],'CUSTOMER_USERNAME_ALREADY_EXISTS') == 0) {
                $response['status'] = 'USER_ALREADY_EXISTS';
            } elseif (strcasecmp($customerKey['status'], 'INVALID_EMAIL') == 0) {
                $response['status'] = 'INVALID_EMAIL';
            } elseif (strcasecmp($customerKey['status'],'INVALID_EMAIL_QUICK_EMAIL') == 0) {
    ...
    function get_current_customer() {
        ...
        if (!empty($content)) {
            ...
            if (!empty($customerKey) && strcasecmp(isset($customerKey['status']), 'ERROR') == 0) {
                $response['status'] = "ERROR";
            } elseif (!empty($customerKey) && !is_array($customerKey) && strpos($customerKey, 'Invalid username or password. Please try again.') !== false) {
                $response['status'] = "INVALID_EMAIL_PASSWORD";
            } elseif (!empty($customerKey) && !is_array($customerKey) && strpos($customerKey, 'The customer is not valid') !== false) {
                $response['status'] = "INVALID_CUSTOMER";
            } elseif (!empty($customerKey) && strcasecmp($customerKey['status'], 'SUCCESS') == 0) {
                $this->save_success_customer_config($customerKey['id'], $customerKey['apiKey'], $customerKey['token'], 'Thanks for registering with the miniOrange');
                update_option('mo_ldap_local_password', '');
                $response['status'] = "SUCCESS";
            }else{
    ...
    function login_widget_save_options()
    {
        if (isset($_POST['option'])) {
            ...
            if ($post_option == "mo_ldap_local_register_customer" and check_admin_referer('mo_ldap_local_register_customer')) {
                $email = '';
                $phone = '';
                $password = '';
                $confirmPassword = '';
                    ...
                    if (!empty($content)) {
                        ...
                        if (strcasecmp($content['status'], 'CUSTOMER_NOT_FOUND') == 0) {
                            ...
                            if (is_array($content) && array_key_exists('status', $content) && $content['status'] == 'INVALID_EMAIL') {
                                ...
                                $this->show_error_message();
                                return;
                            } elseif (is_array($content) && array_key_exists('status', $content) && $content['status'] == 'INVALID_QUICK_EMAIL') {
                                ...
                                $this->show_error_message();
                                return;
                            } elseif (is_array($content) && array_key_exists('status', $content) && $content['status'] == 'USER_ALREADY_EXISTS') {
                                ...
                                $this->show_error_message();
                                return;
                            } elseif (is_array($content) && array_key_exists('status', $content) && $content['status'] == 'ERROR') {
                                ...
                                $this->show_error_message();
                                return;
                            } elseif (is_array($content) && array_key_exists('status', $content) && $content['status'] == 'SUCCESS') {
                            ...
                            if (is_array($response) && array_key_exists('status', $response) && $response['status'] == 'SUCCESS') {
                                ...
                                $this->show_success_message();
                                return;
                            } elseif (is_array($response) && array_key_exists('status', $response) && $response['status'] == 'INVALID_EMAIL_PASSWORD') {
                                $message = 'It seems like you already have an account with miniOrange. Please check username or password and try again.';
                                update_option('mo_ldap_local_message', $message);
                                $this->show_error_message();
                                return;
                            } elseif (is_array($response) && array_key_exists('status', $response) && strcasecmp($response['status'], 'ERROR') == 0) {
                                $message = 'There was an error in registering your account. You can also visit our <a target="_blank" href="https://www.miniorange.com/businessfreetrial">miniOrange Sign Up page</a> to register.';
                                update_option('mo_ldap_local_message', $message);
                                delete_option('mo_ldap_local_admin_email');
                                $this->show_error_message();
            ...
            } elseif ($post_option == "mo_ldap_local_verify_customer" and check_admin_referer('mo_ldap_local_verify_customer')) {
                $email = '';
                $password = '';
                if (Mo_Ldap_Local_Util::check_empty_or_null($_POST['email']) || Mo_Ldap_Local_Util::check_empty_or_null($_POST['password'])) {
                    update_option('mo_ldap_local_message', 'All the fields are required. Please enter valid entries.');
                ...
                if (is_array($content) && array_key_exists('status', $content) && $content['status'] == 'SUCCESS') {
                    ...
                    $this->show_success_message();
                    return;
                } elseif (is_array($content) && array_key_exists('status', $content) && $content['status'] == 'INVALID_EMAIL_PASSWORD') {
                    $message = 'Invalid username or password. Please check username or password and try again.';
                    update_option('mo_ldap_local_message', $message);
                    $this->show_error_message();
                    return;
                } elseif (is_array($content) && array_key_exists('status', $content) && $content['status'] == 'INVALID_CUSTOMER') {
            ...
            elseif ($post_option == "mo_ldap_local_enable" and check_admin_referer('mo_ldap_local_enable') and current_user_can('manage_options')) {

                $enable_ldap_login = (isset($_POST['enable_ldap_login']) and sanitize_text_field($_POST['enable_ldap_login'])==1) ? 1 : 0;
            ...
            } elseif($post_option == 'user_report_logs' and check_admin_referer('user_report_logs')){

                $enable_user_report_logs = (isset($_POST['mo_ldap_local_user_report_log']) and sanitize_text_field($_POST['mo_ldap_local_user_report_log'])==1)? 1 : 0;
            ...
            } elseif ($post_option == "mo_ldap_local_register_user" and check_admin_referer('mo_ldap_local_register_user') and current_user_can('manage_options')) {

                $enable_user_auto_register = (isset($_POST['mo_ldap_local_register_user']) and sanitize_text_field($_POST['mo_ldap_local_register_user'])==1)? 1:0;
            ...
            } elseif ($post_option == "mo_ldap_local_save_config" and check_admin_referer('mo_ldap_local_save_config') and current_user_can('manage_options')) {
                $server_name = '';
                $dn = '';
                $admin_ldap_password = '';
            ...
            } elseif ($post_option == "mo_ldap_local_save_user_mapping" and check_admin_referer('mo_ldap_local_save_user_mapping') and current_user_can('manage_options')) {
                ...
                if (Mo_Ldap_Local_Util::check_empty_or_null($_POST['search_base'])) {
                    ...
                    if (get_option('mo_ldap_local_cust', '1') == '0') {
                        if (strpos($search_base, ';')) {
                            $pricing_url = add_query_arg( array('tab' => 'pricing'), $_SERVER['REQUEST_URI'] );
                            $message = 'You have entered multiple search bases. Multiple Search Bases are supported in the <b>Premium version</b> of the plugin. <a href="' . esc_url($pricing_url) . '">Click here to upgrade</a>.';
                            update_option('mo_ldap_local_message', $message);
                            $this->show_error_message();
                            return;
                        }
                    ...
                    if (!Mo_Ldap_Local_Util::check_empty_or_null($ldap_username_attribute)) {
                        ...
                        if (($ldap_username_attribute == 'custom_ldap_attribute')) {
                            ...
                            if (count($multiple_username_attributes) > 1) {
                                $pricing_url = add_query_arg( array('tab' => 'pricing'), $_SERVER['REQUEST_URI'] );
                                $message = 'You have entered multiple attributes for "Username Attribute" field. Logging in with multiple attributes are supported in the <b>Premium version</b> of the plugin. <a href="' . esc_url($pricing_url) . '">Click here to upgrade</a> ';
                                update_option('mo_ldap_local_message', $message);
                                $this->show_error_message();
                                return;
                            } else {
            ...
            }elseif($post_option == "mo_ldap_save_attribute_config" and check_admin_referer('mo_ldap_save_attribute_config') and current_user_can('manage_options')){
                ...
                update_option( 'mo_ldap_local_message', 'Successfully saved LDAP Attribute Configuration');
                    $this->show_success_message();
            }elseif($post_option == "mo_ldap_local_enable_role_mapping" and check_admin_referer('mo_ldap_local_enable_role_mapping') and current_user_can('manage_options')){

                $enable_role_mapping = (isset($_POST['enable_ldap_role_mapping']) and sanitize_text_field($_POST['enable_ldap_role_mapping'])==1)? 1 : 0;
            ...
            elseif($post_option == "mo_ldap_local_save_mapping" and check_admin_referer('mo_ldap_local_save_mapping') and current_user_can('manage_options')){
            ...
            elseif ($post_option == "mo_ldap_local_test_auth" and check_admin_referer('mo_ldap_local_test_auth')) {
                ...
                elseif (Mo_Ldap_Local_Util::check_empty_or_null($server_name) || Mo_Ldap_Local_Util::check_empty_or_null($dn) || Mo_Ldap_Local_Util::check_empty_or_null($admin_ldap_password) || Mo_Ldap_Local_Util::check_empty_or_null($search_base) || Mo_Ldap_Local_Util::check_empty_or_null($search_filter)) {
                ...
                if (isset($response['statusCode']) && strcasecmp($response['statusCode'], 'LDAP_USER_BIND_SUCCESS') == 0) {
                    $message = 'You have successfully configured your LDAP settings.<br>
								You can set login via directory credentials by checking the Enable LDAP Login in the <b>Sign-In Settings Tab</b> and then <a href="'.esc_url(wp_logout_url( get_permalink() )).'">Logout</a> from wordpress and login again with your LDAP credentials.<br>';
                    update_option('mo_ldap_local_message', $message);
                    $this->show_success_message();
                } elseif (isset($response['statusCode']) && strcasecmp($response['statusCode'], 'LDAP_USER_BIND_ERROR') == 0) {
                    ...
                    update_option('mo_ldap_local_message', $response['statusMessage']);
                    $this->show_error_message();
                } elseif (isset($response['statusCode']) && strcasecmp($response['statusCode'], 'LDAP_USER_SEARCH_ERROR') == 0) {
                    $this->mo_ldap_report_update( sanitize_text_field($_POST['test_username']),'ERROR','<strong>Test Authentication Error: </strong>'.$response['statusMessage']);
                    update_option('mo_ldap_local_message', $response['statusMessage']);
                    $this->show_error_message();
                } elseif (isset($response['statusCode']) && strcasecmp($response['statusCode'], 'LDAP_USER_NOT_EXIST') == 0) {
                    $this->mo_ldap_report_update( sanitize_text_field($_POST['test_username']),'ERROR','<strong>Test Authentication Error: </strong>'.$response['statusMessage']);
                    update_option('mo_ldap_local_message', ($response['statusMessage']));
                    $this->show_error_message();
                } elseif (isset($response['statusCode']) && strcasecmp($response['statusCode'], 'ERROR') == 0) {
                    ...
                    update_option('mo_ldap_local_message', $response['statusMessage']);
                    $this->show_error_message();
                } elseif (isset($response['statusCode']) && strcasecmp($response['statusCode'], 'LDAP_LOCAL_SERVER_NOT_CONFIGURED') == 0) {
                    $this->mo_ldap_report_update(sanitize_text_field($_POST['test_username']),'ERROR','<strong>Test Authentication Error: </strong>'. $response['statusMessage']);
					update_option('mo_ldap_local_message', $response['statusMessage']);
                    $this->show_error_message();
                } else {
            ...
            }elseif($post_option=='mo_ldap_pass' and check_admin_referer('mo_ldap_pass')){
			...
			elseif($post_option=='mo_ldap_export' and check_admin_referer('mo_ldap_export')){
            ...
            } elseif($post_option=='enable_config' and check_admin_referer('enable_config')){
            ...
            }  elseif ($post_option == 'reset_password' and check_admin_referer('reset_password')) {
            ...
            }  elseif ($post_option == 'mo_ldap_local_enable_admin_wp_login' and check_admin_referer('mo_ldap_local_enable_admin_wp_login') and current_user_can('manage_options')) {
                update_option('mo_ldap_local_enable_admin_wp_login', (isset($_POST['mo_ldap_local_enable_admin_wp_login']) and sanitize_text_field($_POST['mo_ldap_local_enable_admin_wp_login'])==1) ? 1 : 0);
            ...
            } elseif ($post_option == 'mo_ldap_local_cancel' and check_admin_referer('mo_ldap_local_cancel')) {
                delete_option('mo_ldap_local_admin_email');
            ...
            } elseif ($post_option == "mo_ldap_goto_login" and check_admin_referer('mo_ldap_goto_login')) {
                delete_option('mo_ldap_local_new_registration');
                update_option('mo_ldap_local_verify_customer', 'true');
            } elseif ($post_option == 'change_miniorange_account' and check_admin_referer('change_miniorange_account')) {
            ...
            }elseif($post_option == 'mo_ldap_call_setup' and check_admin_referer('mo_ldap_call_setup')){
                ...
                $query = '[WP LDAP for Intranet (Free Plugin)]: ' . $query;
                $this->mo_ldap_send_query($email, $phone, $query);
            }elseif ($post_option == 'mo_ldap_plugin_tour_start' and check_admin_referer('mo_ldap_plugin_tour_start')) {
                update_option('mo_tour_skipped','true');
                update_option('overall_plugin_tour','true');
            }elseif ($post_option == 'mo_ldap_skip_ldap_tour' and check_admin_referer('mo_ldap_skip_ldap_tour')) {
                update_option('mo_tour_skipped','true');
                update_option('load_static_UI','false');
            }
            if($post_option =="mo_ldap_trial_request" and check_admin_referer('mo_ldap_trial_request')){
                $phone ='';
            ...
            if ($post_option == 'mo_ldap_skip_feedback' and check_admin_referer('mo_ldap_skip_feedback')) {
                deactivate_plugins(__FILE__);
                update_option('mo_ldap_local_message', 'Plugin deactivated successfully');
                $this->show_success_message();
            }
            if ($post_option == 'mo_ldap_hide_msg' and check_admin_referer('mo_ldap_hide_msg')) {
                update_option('mo_ldap_local_multisite_message', 'true');
            }
            if ($post_option == 'mo_ldap_feedback' and check_admin_referer('mo_ldap_feedback')) {
            ...
            if($mo_option_exists){
                ...
                if($flag==1 )
                    if($value=="mo_ldap_local_server_password" and get_option('mo_ldap_export')== '0')
                        continue;
                    elseif($value=="mo_ldap_local_server_password" and get_option('mo_ldap_export')=='1' )
    ...
    function test_attribute_configuration()
    {
        if (is_user_logged_in()) {
            if (current_user_can('administrator') && isset($_REQUEST['option'])) {
                if ($_REQUEST['option'] != null and sanitize_text_field($_REQUEST['option']) == 'testattrconfig') {
                    $username = sanitize_text_field($_REQUEST['user']);
                    $mo_ldap_config = new Mo_Ldap_Local_Config();
                    $mo_ldap_config->test_attribute_configuration($username);
                } elseif ($_REQUEST['option'] != null and sanitize_text_field($_REQUEST['option']) == 'testrolemappingconfig') {
                    $username = sanitize_text_field($_REQUEST['user']);
                    $mo_ldap_role_mapping = new Mo_Ldap_Local_Role_Mapping();
                    $mo_ldap_role_mapping->test_configuration($username);
                } elseif (sanitize_text_field($_REQUEST['option']) == 'fetchgroups') {
                    $group_search_base = sanitize_text_field($_REQUEST['searchbase']);
                    $mo_ldap_config = new Mo_Ldap_Local_Config();
